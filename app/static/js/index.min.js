...
function getClassNumbers(use_form) {
    ...
    $("#main_container_body").empty()
    $("#main_container_body").html("<h3 class='text-center'>Loading Classes...</h3><div style='text-align: center'><div class='lds-roller' style='margin: auto;'><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div>")
...
function createSemesterReport() {
    $("#card-body").remove()
    $("body").append("<div id='main_container' class='container'><div><div class='jumbotron'><div id='main_container_body'><h3 class='text-center'>Creating semester report...</h3><div style='text-align: center'><div class='lds-roller' style='margin: auto;'><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div></div></div></div>")
...
function getSpecificClass(subject, class_number, class_section) {
    semester = $("#semester").text()
    year = $("#year").text()
...
function deleteOverride(override_id, class_number, class_section) {
    ...
    $.get(url, payload, function (response) { display_notification(response) }).done(function () {
...
function ChangeRegistrationStatus(override_id) {
    ...
    $.post(url, payload, function (response) {
        ...
        my_table = $("#overrideTable").DataTable()
        my_table.destroy(remove = false)
        $("#potentially_enrolled").html(response["potentially_enrolled"])
...
function alterMaxCapacity(id) {
    ...
    $.get(url, payload, function (response) { display_notification(response) })
        ...
        var validation = Array.prototype.filter.call(forms, function (form) {
            form.addEventListener('submit', function (event) {
                if (form.checkValidity() === false) {
                    ...
                    // Remove Select Semester
                    $("#main_container_body").empty()
                    $("#catsweb_semesterModal").modal('hide')
                    $("#main_container_body").html("<h3 class='text-center'>Creating semester...</h3><div style='text-align: center'><div class='lds-roller' style='margin: auto;'><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div>")
                    ...
                    $.post(url, payload, function (response) { 
                        ...
                        $("#main_container_body").empty()
                        $("#main_container_body").html("<h1 class='text-center'>Select Semester</h1><div class='text-left'><div class='input-group mb-3'><div class='input-group-prepend'><label class='input-group-text' for='inputGroupSelect01'>Semester</label></div><select id='inputGroupSelect01' class='custom-select'></select></div><div class='input-group mb-3'><div class='input-group-prepend'><label class='input-group-text' for='inputGroupSelect01'>Year</label></div><select id='my_years' class='custom-select'></select></div><button onclick='getClassNumbers(true)' class='btn btn-primary' type='button'>Go</button></div>")
...
(function () {
    ...
    window.addEventListener('load', function () {
        // Fetch all the forms we want to apply custom Bootstrap validation styles to
        var forms = document.getElementsByClassName('new_class_form');
        // Loop over them and prevent submission
        var validation = Array.prototype.filter.call(forms, function (form) {
            form.addEventListener('submit', function (event) {
                if (form.checkValidity() === false) {
                    ...
                    $("#overrideModal").modal('hide')
                    event.preventDefault()
                    var $form = $("#new_class_form")
                    var url = $form.attr('action')

                    var payload = {
                        "class_number": $("#new_class_number").val(),
                        "class_section": $("#new_class_section").val(),
                        "max_capacity": $("#new_class_max_capacity").val(),
                        "semester": $("#semester").text(),
                        "year": $("#year").text(), 
                        "subject": $("#new_class_subject").val()
                    }
